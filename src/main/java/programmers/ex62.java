    ...
    public static int[] solution(int[] fees, String[] records) {
        ...
        for (String r : records) {
            ...
            if (record[2].equals("IN")) {
                map.put(record[1], curTime);
            } else {
                map2.put(record[1], map2.getOrDefault(record[1], 0) + curTime - map.get(record[1]));
                map.put(record[1], -1);
            }
        }
        ...
        for (String k : map.keySet()) {
            if (map.get(k) != -1) {
                map2.put(k, map2.getOrDefault(k, 0) + (23 * 60 + 59) - map.get(k));
                map.put(k, -1);
            }
        }
        ...
        for (String k : keys) {
            int time = map2.get(k);
            if (basicM < time) {
                temp.add(basicF + (int)(Math.ceil((double)(time - basicM) / (double) standard)) * standardF);
            } else {
                temp.add(basicF);
            }
        }
