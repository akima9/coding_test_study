...
public class ex55 {
    ...
    public static int solution(int k, int[][] dungeons) {
        ...
        // 최대 Level(깊이)을 구하면 될듯
        dungeon = dungeons;
        check = new int[dungeon.length];
        DFS(k, 0);
    ...
    public static void DFS(int presentK, int level) {
        for (int i = 0; i < dungeon.length; i++) {
            if (check[i] != 1 && presentK >= dungeon[i][0]) {
                check[i] = 1;
                DFS(presentK - dungeon[i][1], level + 1);
                check[i] = 0;
            }
        }
        L = Math.max(L, level);
