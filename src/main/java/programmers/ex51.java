...
public class ex51 {
    ...
    public static int solution(String[] want, int[] number, String[] discount) {
        ...
        for (int i = 0; i < discount.length; i++) {
            ...
            int len = i + 10;
            if (len > discount.length) len = discount.length;

            for (int j = i; j < len; j++) {
                String curDiscount = discount[j];
                if (map.containsKey(curDiscount) && map.get(curDiscount) > 0) {
                    map.put(curDiscount, map.get(curDiscount) - 1);
                }
            }
            boolean flag = true;
            for (String key : map.keySet()) {
                if (map.get(key) != 0) flag = false;
            }
            if (flag) answer++;
        }
        return answer;
