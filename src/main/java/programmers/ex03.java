...
public class ex03 {
    public static void main(String[] args) {
        System.out.println(solution(8, 2, new int[]{2, 3, 6}));
    }
    ...
    public static int solution(int n, int m, int[] section) {
        int answer = 0;
        int painted = 0; // 색칠한 부분 중 끝부분

        // 빈벽의 개수 만큼 반복
        for (int wall : section) {
            if (painted >= wall) continue; // 직전의 빈벽을 칠하면서 이미 칠해진 빈벽은 pass
            painted = m + wall - 1; // 빈벽을 포함해서 롤러의 크기 만큼 칠 할 수 있으므로 (롤러 + 빈벽 번호 - 1)을 해준다.
            answer++;
        }
