...
public class ex06 {
    ...
    public static int[] solution(String[] park, String[] routes) {
        ...
        for (String x : routes) {
            queue.offer(new Route(x.charAt(0), Character.getNumericValue(x.charAt(2))));
        }
        ...
        while (queue.size() > 0) {
            ...
            int hTemp = sh;
            if (poll.id == 'E') {
                for (int i = 0; i < poll.run; i++) {
                    sw++;
                    if (sw < 0 || sw >= W || parks[sh][sw] == 'X') {
                        sw = wTemp;
                        break;
                    }
                }
