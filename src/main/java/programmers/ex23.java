...
public class ex23 {
    ...
    public static int[] solution(String[] wallpaper) {
        /**
         * 시작점의 행: #이 있는 행 중 가장 위의 행 (행의 최소값)
         * 시작점의 열: #이 있는 열 중 가장 왼쪽 열 (열의 최소값)
         * 도착점의 행: #이 있는 행 중 가장 아래 행 + 1 (행의 최대값 + 1)
         * 도착점의 열: #이 있는 열 중 가장 오른쪽 열 + 1 (열의 최대값 + 1)
         */
        int[] answer = new int[4];
        int lux = Integer.MAX_VALUE; // 시작점의 행
        int luy = Integer.MAX_VALUE; // 시작점의 열
        int rdx = Integer.MIN_VALUE; // 도착점의 행
        int rdy = Integer.MIN_VALUE; // 도착점의 열

        for (int i = 0; i < wallpaper.length; i++) {
            int isFile = wallpaper[i].indexOf('#');
            if (isFile >= 0) {
                lux = Math.min(lux, i);
...
//
//머쓱이의 컴퓨터 바탕화면의 상태를 나타내는 문자열 배열 wallpaper가 매개변수로 주어질 때 바탕화면의 파일들을 한 번에 삭제하기 위해 최소한의 이동거리를 갖는 드래그의 시작점과 끝점을 담은 정수 배열을 return하는 solution 함수를 작성해 주세요. 드래그의 시작점이 (lux, luy), 끝점이 (rdx, rdy)라면
